...
export default function MapBottomSheet({ procesionId }: { procesionId?: string }) {
  ...
  const centerMap = () => {
    if (window.map) {
      try {
        if (window.animatedMarker && typeof window.animatedMarker.getLatLng === 'function') {
          const centerPos = window.animatedMarker.getLatLng();
          window.map.setView(centerPos, 18);
  ...
  return (
    <>
      ...
      >
        ...
        >
          {!panelVisible ? (
            // Cuando el panel está abierto mostramos el icono proporcionado (map search)
            <React.Fragment>
            ...
            // Cuando está cerrado mostramos el icono de cerrar (cruz)
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 6l12 12M6 18L18 6" />
          )}
        </motion.svg>
      </button>

      {/* Conjunto de panel y botones, con animación al aparecer y desaparecer */}
      <AnimatePresence>
        {panelVisible && (
          <motion.div
            ...
            // Se baja un poco más el tablero y se ajusta el espaciado
            className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40 w-11/12 max-w-sm space-y-2"
          >
            {/* Panel negro flotante con bordes redondeados */}
